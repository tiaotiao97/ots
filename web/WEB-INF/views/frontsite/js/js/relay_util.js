var FeedbackUtil = {
	feed:function (targetUserId,msgId) {
		var stylehtml = '<style>';
		...
		html +='<!-- 登录框 s -->';
			html +='<ul class="pop_login" style="padding-left:10px;">';
				...
				html +='</li>';
				html +='<li style="height:200px;width:100%"><span class="l">回复消息：</span>';
					html +='<span class="r" style="width:400px;height:100%">';
						html +='<span class="input" style="width:400px;height:180px">';
							html +='<textarea tabindex="2" onclick="FeedbackUtil.onfocus();" style="width:400px;height:180px;color:#999999;font-size:14px;line-height:18px;" rows="" cols="" id="fb_content" name="content" >请在这里输入您回复内容。</textarea>';
							html +='<input type="hidden" name="targetUserId" value='+targetUserId+'>';
        					html +='<input type="hidden" name="msgId" value='+msgId+'>';
						html +='</span>';
					html +='</span>';
				html +='</li>';
				html +='<li ><p style="padding-left:90px;color:#999999" id="content_tip">回复内容请控制在1000字以内</p></li>';
				html +='<li class="zc">';
					html +='<span class="l">&nbsp;</span>';
					html +='<input type="submit" onclick="FeedbackUtil.send(\''+'\')" value="提    交" class="button" tabindex="7">';
				html +='</li>';
			html +='</ul>';
			html +='</div>';
			html +='<!-- 登录框 e -->';
	...
	send:function(targetUserId) {
		...
		if(content == '') {
			...
			$("#content_tip").html('回复内容不能为空');
			return '';
		}

		var url = window.location.href;
		FeedbackUtil.tip('succ', '您的回复提交成功!');
		FeedbackUtil.close();
	...
	onfocus:function(){
		if($("#fb_content").val() == '请在这里输入您的回复内容。'){
			$("#fb_content").val('')
		}
